...
(e.g. `someones_drive`) or the connection UUID. Both resolve. Examples below
use `someones_drive`; substitute the connection you were actually granted.
...
```
hearth resource invoke someones_drive list_files
hearth resource invoke someones_drive get_file_metadata '{"file_id":"<id>"}'
```
...
```
hearth resource invoke someones_drive search_files '{"query":"name contains '\''budget'\''"}'
hearth resource invoke someones_drive search_files '{"query":"fullText contains '\''Q3 revenue'\''"}'
hearth resource invoke someones_drive search_files '{"query":"mimeType = '\''text/plain'\''"}'
```
...
```
hearth resource invoke someones_drive download_file '{"file_id":"<id>"}'
```
...
hearth resource invoke someones_drive export_file '{"file_id":"<id>","mime_type":"text/plain"}'

# Google Doc → Word
hearth resource invoke someones_drive export_file '{"file_id":"<id>","mime_type":"application/vnd.openxmlformats-officedocument.wordprocessingml.document"}'

# Google Sheet → CSV
hearth resource invoke someones_drive export_file '{"file_id":"<id>","mime_type":"text/csv"}'

# Any file → PDF
hearth resource invoke someones_drive export_file '{"file_id":"<id>","mime_type":"application/pdf"}'
```
...
hearth resource invoke someones_drive create_file '{
  "name": "notes.md",
...
```
hearth resource invoke someones_drive upload_file_content '{
  "file_id": "<id>",
  "mime_type": "text/markdown",
  "content": "# My document\n\nContent here."
}'
...
Rename:
```
hearth resource invoke someones_drive rename_file '{"file_id":"<id>","name":"New name.md"}'
```

Move (requires the current parent ID — get it from `get_file_metadata`):
```
hearth resource invoke someones_drive move_file '{
  "file_id": "<id>",
...
```
hearth resource invoke someones_drive trash_file '{"file_id":"<id>"}'
```
...
hearth resource invoke someones_drive share_file '{
  "file_id": "<id>",
  "role": "writer",
  "type": "user",
