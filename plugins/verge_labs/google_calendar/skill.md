...
(e.g. `someones_calendar`) or the connection UUID. Both resolve. Examples
below use `someones_calendar`; substitute the connection you were actually
granted.

This plugin authenticates as a service account impersonating one Workspace
user, so the connection acts as *that person's* calendar.
...
```
hearth resource invoke someones_calendar list_calendars
hearth resource invoke someones_calendar list_events '{"calendar_id":"primary"}'
```
...
```
hearth resource invoke someones_calendar check_availability '{"time_min":"2026-07-20T09:00:00-07:00","time_max":"2026-07-20T18:00:00-07:00","items_json":"[{\"id\":\"alice@example.com\"}]"}'
```
...
```
hearth resource invoke someones_calendar create_event '{
  "calendar_id": "primary",
...
```
hearth resource invoke someones_calendar list_events '{
  "calendar_id": "primary",
...
```
hearth resource invoke someones_calendar get_event '{"calendar_id":"primary","event_id":"<id from list_events>"}'
```
...
hearth resource invoke someones_calendar get_event '{"calendar_id":"primary","event_id":"<id>"}'

# 2. Update with all fields (changed + unchanged)
hearth resource invoke someones_calendar update_event '{
  "calendar_id": "primary",
  "event_id": "<id>",
...
```
hearth resource invoke someones_calendar cancel_event '{"calendar_id":"primary","event_id":"<id>"}'
```
